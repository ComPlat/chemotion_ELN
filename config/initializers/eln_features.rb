...
messages = {
  ...
  extension_not_installed: 'WARN PG cartridge extension available but not installed !! feature enabled but ensure the extensiont is installed',
...
ActiveSupport.on_load(:active_record) do
  ...
  Rails.configuration.pg_cartridge = case
                                     ...
                                     when valid && !available
                                       output_message.call(:warn, messages[:extension_not_available], cartridge, version)
                                       'none'
                                     when available && !installed
                                       output_message.call(:warn, messages[:extension_not_installed], cartridge, version)
                                       cartridge
                                     when installed
