...
module SVG
  class ReactionComposer
    ...
    def getInnerFileContent fileName
    ...
    def compose_reaction_svg
      ...
      output += '</svg>'
      svg = Nokogiri::XML(output)
    end

    def compose_reaction_svg_and_save
      svg = compose_reaction_svg
      file_path = File.join(File.dirname(__FILE__), '..', '..', 'public', 'images', 'reactions', generateFilename)
      File.open(file_path, 'w') { |file| file.write(svg) }
    end
  end
end

composer = SVG::ReactionComposer.new ["AFABGHUZZDYHJO-UHFFFAOYSA-N", "KIWAUQFHKHLABA-AATRIKPKSA-N", "XLYOFNOQVPJJNP-UHFFFAOYSA-N"],
