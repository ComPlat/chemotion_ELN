...
module Analyses
  class Converter
    def self.logger
      @@converter_logger ||= Logger.new(File.join(Rails.root, 'log', 'converter.log'))
    end
    ...
    def self.vor_conv(id)
      ...
      return nil if conf.nil? || oa.nil? || folder.nil?
    ...
    def self.collect_metadata(zip_file)
      ...
      zip_file.each do |entry|
        ...
        matches&.each do |match|
          ...
          idr = match['result']
          dsr.push(layer: idf['outputLayer'], field: idf['outputKey'], value: idr['value']) if idf&.class == Hash && idr&.class == Hash && !idf['outputLayer'].nil? && !idf['outputKey'].nil? && !idr['value'].nil?
        end
      end
      { d: dsr, o: ols }
    end
    ...
    def self.handle_response(oat, response)
      ...
      begin
        ...
        name = response&.headers['content-disposition']&.split('=')&.last
        att = Attachment.new(
          filename: name,
        ...
        dsr.flatten!
        Analyses::Converter.ts('write', att.attachable_id, ols: ols, info: dsr) if dsr.length.positive? && name.split('.')&.last == 'zip'
    ...
    def self.build_ds(att_id, ols)
      ...
      Dataset.create!(
        ...
        element_id: cds.id,
        properties: props,
        ## properties_release: klass.properties_release,  ## TODO
