...
module Import
  ...
  class ImportSamples
    ...
    def read_file
      ...
      @sample_names_with_components = extract_sample_names_with_components if component_sheet_exists?
    end

    def extract_sample_names_with_components
      # Find the column index for the header "sample name"
      sample_name_col = component_header.index('sample name') + 1

      # Collect values from that column, skipping the header
      component_sheet.column(sample_name_col).drop(1).compact.uniq
    end
    ...
    def parse_sample_components_data
      ...
      (2..component_sheet.last_row).each do |row_index|
        ...
        first_cell = row_values[0].to_s.strip

        if first_cell && !first_cell.strip.empty?
          # This is a new sample name
          current_sample = first_cell
          @sample_components_data[current_sample] = []
        end

        next unless current_sample # skip rows until a sample name is found
        ...
        component_attributes = component_data.reject do |k, _|
          k.to_s.downcase.strip.in?(['sample name', 'sample external label'])
        end

        @sample_components_data[current_sample] << component_attributes if valid_component_data?(component_attributes)
      end
    end
    ...
    def validate_sample_and_save(sample, stereo, row)
      ...
      sample.sample_type = 'Mixture' if sample_has_components?(sample)
      sample.save!
      save_chemical(chemical, sample) if @import_type == 'chemical'
      handle_sample_components(sample) if sample_has_components?(sample)
      processed.push(sample)
    end

    def sample_has_components?(sample)
      sample&.name.present? && @sample_names_with_components.include?(sample.name)
    end

    def handle_sample_components(sample)
      sample_components_data = @sample_components_data.with_indifferent_access[sample.name]
