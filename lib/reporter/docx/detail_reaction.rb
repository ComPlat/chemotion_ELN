  ...
  module Docx
    class DetailReaction < Detail
      ...
      def calculate_mixture_amount_mol(sample, mass)
        ...
        if has_reference_changed
          # Case 1: Initial case - when the reference has been changed
          (ref_amount_mol || 0).to_f
        elsif rel_mol_weight&.to_f&.positive?
      ...
      def component_metric_pref(comp_props, index, metric_prefixes, parent_material = nil)
        ...
        if metrics.nil? || metrics.empty?
          metrics = parent_material&.metrics || 'mmmm'
        end

        # Default to 'm' if still not found
        return 'm' if metrics.nil? || metrics.empty?
        ...
        if material.sample_type == Sample::SAMPLE_TYPE_MIXTURE || material.components.present?
          ...
          if components.present? && components.any?
            sample_hash[:components] = components.map do |comp|
              ...
              # Create OpenStruct for parent material to access metrics
              parent_material = OpenStruct.new(material)

              # Get converted amounts and units using similar functions as normal samples
              amount_mol_value, concn_value = component_assigned_amount(comp_props, parent_material)
              amount_mol_unit, concn_unit = component_unit_conversion(comp_props, parent_material)
