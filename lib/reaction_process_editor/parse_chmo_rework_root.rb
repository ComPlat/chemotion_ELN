...
class ParseChmo
  ...
  def run(doc)
    CHMO_IDS.each do |chmo_id|
      ...
      write_csv(nodes: nodes,
                filename: "#{chmo_id}_#{parse_label(nodes.flatten.first)}.csv")
    end.flatten
  end
  ...
  def write_csv(nodes:, filename:)
    headers = ['CHMO', 'Ontology Name', 'Own Name', 'Full Link', 'Parent', 'Marker']
    CSV.open("chmo_ontologies/#{filename}", 'w',
             write_headers: true,
             headers: headers,
             col_sep: ';') do |csv|
