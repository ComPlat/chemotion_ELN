...
module DataCite
    class LiteraturePaser
      def self.parse_bibtex!(bib, id)
        ...
        Rails.logger.error ["BibTeX parse error, Literature Id: #{id}", e.message, *e.backtrace].join($INPUT_RECORD_SEPARATOR)
      end

      def self.doi_url(id, url, doi, bib)
        return '' if url.empty? && doi.empty? && bib.empty?

        resp = url.blank? ? (bib['url']&.to_s) : url
      ...
      def self.get_metadata(bib, doi, id)
        return {} if doi.empty?
        ...
        Rails.logger.error ["DataCite metadata error id: #{id}, doi: #{doi}", ex.message, *ex.backtrace].join($INPUT_RECORD_SEPARATOR)
      end
      ...
      def self.report_hash(lit, bib)
        url = LiteraturePaser.doi_url(lit[:id], lit[:url], lit[:doi], bib)
        { title: lit[:title], url: url, doi: lit[:doi], year: bib['year']&.to_s, issn: bib['issn']&.to_s, author: bib['author']&.to_s }
      rescue StandardError => e
        { title: lit[:title], url: url, doi: lit[:doi], year: bib['year']&.to_s, issn: bib['issn']&.to_s, author: bib['author']&.to_s&.dup&.force_encoding('UTF-8') }
      end
    end
  end
