...
module PubChem
  ...
  def self.get_record_from_molfile(molfile)
    ...
    options = { :timeout => 5,  :headers => {'Content-Type' => 'application/x-www-form-urlencoded'}, :body => { 'sdf' => molfile } }
  ...
  def self.get_record_from_inchikey(inchikey)
    @auth = {:username => '', :password => ''}
    options = { :timeout => 6,  :headers => {'Content-Type' => 'text/json'}  }
  ...
  def self.get_molfile_by_inchikey(inchikey)
    @auth = {:username => '', :password => ''}
    options = { :timeout => 5,  :headers => {'Content-Type' => 'text/json'}  }
