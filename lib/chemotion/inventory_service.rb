...
module Chemotion::InventoryService
  ...
  def self.chemical_properties_alfa(product_link)
    ...
    begin
      ...
      # properties.map { |str| str.gsub('(lit.)', '') }
      chemical_properties = {}
      properties.each_with_index do |property, index|
        property_name = property.gsub(' ', '_').downcase
        chemical_properties[property_name] = properties[index + 1] unless index.odd?
      end
      # if chemical_properties['boiling_point']
      #   chemical_properties['boiling_point'] = chemical_properties['boiling_point'].gsub('(lit.)', '')
      #   puts chemical_properties
  ...
  def self.chemical_properties_merck(product_link)
    ...
    begin
      ...
      chem_properties_values = properties.search('p.MuiTypography-root').map(&:text).map { |str| str.gsub('(lit.)', '') }
