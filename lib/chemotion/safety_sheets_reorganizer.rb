...
class SafetySheetsReorganizer
  ...
  # @example
  #   process_file("/app/public/safety_sheets/merck_product123.pdf")
  #   # Moves to: /app/public/safety_sheets/merck/merck_product123.pdf
  ...
  # @example
  #   rename_file_remove_vendor_prefix("/app/merck/merck_product123.pdf", "merck")
  #   # Result: /app/merck/product123_a1b2c3d4e5f6.pdf
  def rename_file_remove_vendor_prefix(file_path, vendor_name)
    ...
    # Add initials to filename: productNumber_initials.pdf
    final_filename = "#{new_filename}_#{hash_initials}"
  ...
  #   # Creates: /app/merck/prod_v1_abc123.pdf (or higher version if v1 exists)
  def create_versioned_file(source_file_path, target_template_path, base_filename, source_hash)
    # Extract product name and hash from base_filename (format: product_hash)
    # We need to insert version between product and hash
    if base_filename =~ /^(.+)_([a-f0-9]{16})$/
      product_name = Regexp.last_match(1)
      hash_initials = Regexp.last_match(2)
    else
      # Fallback: treat entire base_filename as product_name and generate new hash
      product_name = base_filename
      hash_initials = Chemotion::GenerateFileHashUtils.generate_file_hash_initials(source_file_path, base_filename)
    end

    version = 1

    loop do
      # Create versioned filename: product_v1_hash.pdf
      versioned_filename = "#{product_name}_v#{version}_#{hash_initials}"
