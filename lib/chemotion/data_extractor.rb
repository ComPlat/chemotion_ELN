...
module Chemotion
  ...
  module DataExtractor
    ...
    # Parse complete filename into components
    # @param filename [String] Complete filename to parse
    # @return [Hash] Hash containing extracted components or error information
    # @note Returns hash with keys: :vendor, :product, :hash_initials, :valid, :error
    # @example
    #   parse_filename("merck_prod123_a1b2c3d4e5f6.pdf")
    #   #=> { vendor: "merck", product: "prod123", hash_initials: "a1b2c3d4e5f6", valid: true }
    #
    #   parse_filename("invalid_format.pdf")
    #   #=> { vendor: nil, product: nil, hash_initials: nil, valid: false, error: "Invalid format" }
=begin     
    def self.parse_filename(filename)
      ...
      return result.merge(error: "Invalid or missing vendor") unless vendor

      # Extract product number
      product = extract_product_number_from_filename(filename, vendor)
      return result.merge(vendor: vendor, error: "Invalid or missing product number") unless product

      # Extract hash initials
      hash_initials = extract_hash_initials_from_filename(filename)
      return result.merge(vendor: vendor, product: product, error: "Invalid or missing hash initials") unless hash_initials

      # All components successfully extracted
      result.merge(
        vendor: vendor,
        product: product,
        hash_initials: hash_initials,
        valid: true
      )
    end
=end
