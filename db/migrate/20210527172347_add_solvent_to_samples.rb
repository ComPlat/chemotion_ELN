class AddSolventToSamples < ActiveRecord::Migration
  def change
    ...
    Sample.where(solvent: [nil, '']).where.not(deprecated_solvent: [nil, '']).each do |item|
      solvent = solvents_smiles_options.find { |s| s[:label].include?(item.deprecated_solvent) }
      puts item.deprecated_solvent if solvent.blank?
      item.solvent = [{ label: solvent[:label], smiles: solvent[:value][:smiles], ratio: '100' }.to_json]
      item.save!
    end
  end
end
