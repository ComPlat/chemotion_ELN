...
def create_vessels_and_vessel_templates
  ...
  ActiveRecord::Base.transaction do
    vessel_names.each_with_index do |vessel_name, index|
      vessel_template = VesselTemplate.find_or_create_by!(name: vessel_name) do |vt|
        vt.details = Faker::Science.element_subcategory
        vt.material_details = Faker::Science.tool
        vt.material_type = vessel_materials.sample
        vt.vessel_type = ord_vessel_types.sample
        vt.volume_amount = index < 21 ? small_vessel_sizes.sample : large_vessel_sizes.sample
        vt.volume_unit = 'ml'
        vt.weight_amount = weight_amounts.sample
        vt.weight_unit = 'g'
      end
