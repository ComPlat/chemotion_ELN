...
#              -> set remaining samples as decoupled and set the mw
#
...
class MoleculeSeed
  ...
  def process_samples_by_sample_mol(molecule)
    ...
    get_samples(molecule).each do |sample|
      ...
      if !new_molecule.present? || new_molecule.id == sample.molecule_id
        logger.info "Molecule not found or already associated with sample with id: #{sample.id}"
        next
      end
      sample.update_columns(molecule_id: new_molecule.id)
      logger.info "Updated sample with id: #{sample.id} with molecule id: #{new_molecule.id}"
    end
  end
  ...
  def process_samples_by_molecule_smiles(molecule)
    ...
    get_samples(molecule_id).each { |sample| sample.update_columns(molecule_id: new_molecule.id) }
  end
  ...
  def samples_empty?(molecule_id)
    ...
    logger.info "Molecule with id: #{molecule_id} has no samples left and will be deleted" if result
  end

  # define local logger to write to log/seed.log:
  def logger
    @logger ||= Logger.new(Rails.root.join('log/seed.log'))
  end

  # Find problematic molecules
  def faulty_molecules
    count = Molecule.where('molfile LIKE ?', 'Status: %').count
    logger.info "Found #{count} molecules with molfile starting with 'Status:%'"
    Molecule.where('molfile LIKE ?', 'Status:%')
  end
  ...
  def get_rdkit_ctab_by_smiles(input)
    ...
    logger.info("Failed to convert smiles to molfile using rdkit, smiles: #{smiles}") if ctab.blank?
  rescue StandardError => e
  ...
  def get_inchikey(obj)
    ...
    # this use Inchi gem
    inchikey = Chemotion::OpenBabelService.inchi_info(molfile)&.fetch(:inchikey, nil)
  rescue StandardError => e
