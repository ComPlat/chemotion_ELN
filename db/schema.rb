...
ActiveRecord::Schema.define(version: 2023_05_11_115408) do
  ...
  create_table "disk_usages", force: :cascade do |t|
    ...
    t.datetime "updated_at"
    t.datetime "deleted_at"
    t.index ["user_id"], name: "index_disk_usages_on_user_id"
  end
  ...
  create_view "mat_disk_usages", materialized: true, sql_definition: <<-SQL
      SELECT COALESCE(sum(disk_usages.filesize), (0)::bigint) AS total
     FROM disk_usages
    WHERE (disk_usages.user_id IS NULL);
  SQL
