...
export class AttachmentTab extends Component {
  ...
  componentDidUpdate(prevProps) {
    const prevLen = (prevProps.attachments || []).length;
  ...
  getSortedFilteredAttachments() {
    const { filterText, sortBy, sortDirection } = this.state;
    const list = (this.props.attachments || []).filter(
      (a) => a.filename && a.filename.toLowerCase().includes(filterText.toLowerCase())
    );
    list.sort((a, b) => {
      let comparison = 0;
      switch (sortBy) {
