...
class ContainerWrapper {
    ...
    constructor({id, name, description, children, files}) {
...
function AnalysisListItem({backgroundColor, container, setContainer, addContainer, removeContainer}) {

    return (<ListGroup.Item style={{
        backgroundColor: backgroundColor
    }} key={'item_' + container.id} className="d-flex justify-content-between align-items-start">

        <div key={`DIVV__${container.id}`} className="flex-grow-1 me-3"
            ...
            {container.children.map((ds) => {
                return <DatasestListItem
                    key={ds.id}
                    dataset={ds}
                    droppedPaths={ds.files}
                    removeDataset={(datasetId) => setContainer((x) => {
                        const children = x.children.filter((child) => child.id !== datasetId).map((child, idx) => {
                            return new ContainerWrapper({...child, name: `Dataset #${idx + 1}`});
                        });
                        return {...x, children};
                    })}
...
function AdvancedAnalysesList({handleClose, setConsumedPaths, listedFiles, setElement}) {
    const [analContainerList, setAnalContainer] = useState([]);
    const wrapperSetAnaContiainer = (val) => {
        let changedVal = typeof val === 'function' ? val(analContainerList) : val;
    ...
    const execute = useCallback(() => {
        setElement((element) => {
            ...
            const preprocesdAnaContainer = analContainerList.map((ana) => {
                ...
                let anaContainer = addNewAnalyses(element);
    ...
    return (<>
        ...
        <ListGroup
            ...
            {analContainerList.map((anaContainer, index) => {
                ...
                return <AnalysisListItem backgroundColor={index % 2 === 0 ? '#d8d9da' : '#c8c9ca'}
                                         key={anaContainer.id}
                                         container={anaContainer}
                                         removeContainer={removeContainer}
                                         addContainer={addDataset}
                                         setContainer={setContainer}></AnalysisListItem>
            })}
        </ListGroup></>)
}
