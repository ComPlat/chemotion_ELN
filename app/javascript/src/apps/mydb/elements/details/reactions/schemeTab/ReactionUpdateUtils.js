...
export default class ReactionUpdateHandler {
  ...
  set reaction(reaction) {
    ...
    this.updateReactionEquivLockState();
  }
  get lockEquivColumn() {
    if (this._lockEquivColumn === null) {
      this._lockEquivColumn = this.getReactionEquivLockState(this.reaction);
      this.props.onLockEquivColChange(this._lockEquivColumn);
    }

    return this._lockEquivColumn;
  }
  set lockEquivColumn(lockEquivColumn) {
    this._lockEquivColumn = lockEquivColumn;
    this.props.onLockEquivColChange(this._lockEquivColumn);
  }

  updateReactionEquivLockState() {
    this._lockEquivColumn = null;
    return this.lockEquivColumn;
  }
  ...
  switchEquiv() {
    ...
    this.lockEquivColumn = newLockState;
  }
  ...
  handleMaterialsChange(changeEvent) {
    ...
    switch (changeEvent.type) {
      ...
      case 'externalLabelCompleted':
        onReactionChange(this.reaction, { updateGraphic: true });
        break;
      case 'addToDesc':
  ...
  guardConcentrationUpdate(reaction) {
    ...
    if (reaction.canUpdateConcentration(this.lockEquivColumn)) {
      return true;
    }
  ...
  updatedSamplesForAmountChange(samples, updatedSample, materialGroup) {
    ...
    return samples.map((sample) => {
      ...
      reaction.updateReferenceAmountForLockedEquivalents(sample, updatedSample, this.lockEquivColumn);
      ...
      if (referenceMaterial) {
        if (sample.id === updatedSample.id) {
          ...
          if ((!this.lockEquivColumn || materialGroup === 'products') && sample.gas_type !== 'gas') {
            ...
            if (referenceMaterial &&
              referenceMaterial.amount_value &&
              updatedSample.gas_type !== 'feedstock' &&
              sample.gas_type !== 'gas') {
              sample.setAmountAndNormalizeToGram({
                value: sample.equivalent * referenceMaterial.amount_mol,
      ...
      if (isReferenceMatch) {
        ...
        if (sample.amount_value || sampleAmountMol) {
          if (referenceMaterial && referenceAmountMol > 0) {
            ...
            referenceMaterial && !sample.reference) {
