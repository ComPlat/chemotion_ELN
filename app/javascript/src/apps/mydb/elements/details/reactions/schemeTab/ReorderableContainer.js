...
export default function ReorderableContainer({
  materials,
  materialGroup,
  onDrop,
  onReorder,
  ...
  const contents = renderedMaterials.map((material, index) => (
    <Item
      key={`${material.id}${hoveringElement && hoveredIndex === index ? '-new' : ''}`}
      material={material}
...
ReorderableContainer.propTypes = {
...
ReorderableContainer.defaultProps = {
  className: null,
};
...
function Item({
  ...
  const [{ isOver, isHoveredMaterial, canDrop }, drop] = useDrop({
    accept: [
      DragDropItemTypes.MATERIAL,
      ...newElementTypes,
    ],
    collect: (monitor) => ({
      isOver: monitor.isOver(),
      isHoveredMaterial: monitor.getItem()?.material?.id === material.id,
      canDrop: [
        DragDropItemTypes.MATERIAL,
        ...newElementTypes
  ...
  return renderMaterial({
    ...
    // We may indeed be hovering over the dragged material, but could also be
    // hovering over a preview of the dragged material in another list. In both
    // cases, we want to show the item as being dragged.
    isDragging: isDragging || isHoveredMaterial,
  });
}
