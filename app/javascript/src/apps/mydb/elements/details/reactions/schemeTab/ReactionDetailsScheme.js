    ...
    if (reaction.editedSample !== undefined) {
      ...
      if (reaction.editedSample.amountType === 'target') {
        materialGroups.forEach((group) => {
          reaction[group] = this.reactionUpdateHandler.updatedSamplesForEquivalentChange(reaction[group] || [], reaction.editedSample, group);
        });
      } else { // real amount, so that we update amount in mmol
        materialGroups.forEach((group) => {
          reaction[group] = this.reactionUpdateHandler.updatedSamplesForAmountChange(reaction[group] || [], reaction.editedSample, group);
        });
      }
