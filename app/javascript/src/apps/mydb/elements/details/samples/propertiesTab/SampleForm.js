...
export default class SampleForm extends React.Component {
  ...
  totalMixtureMass() {
    const { sample } = this.props;
    // For mixtures, prioritize total_mixture_mass_g (calculated value) over amount_g
    // The getter returns null if not set, so we check for both null and undefined
    const totalMixtureMass = sample.total_mixture_mass_g;
    const massG = (totalMixtureMass != null && totalMixtureMass !== undefined)
      ? totalMixtureMass
      : (sample.amount_g || 0);
