    ...
    module Samples
      class SampleInActionExporter
        ...
        def ontology
          ...
          OrdKit::Exporter::Models::OntologyExporter.new(action.ontology.ontology_id).to_ord
        end
        ...
        def preparations
          [SamplePreparationsExporter.new(action).to_ord].compact
        end

        def purity
          value = (action.sample&.purity || 1) * 100
          OrdKit::Percentage.new(value: value)
        end
