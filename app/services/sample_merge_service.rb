...
class SampleMergeService
  ...
  def apply_merge!(source, target, reaction)
    ...
    rps_attributes = destroy_source_reaction_product!(source, reaction)
    ...
    SampleMerge.create!(
      ...
      source_reaction_product_attributes: rps_attributes,
    )
  end
  ...
  def destroy_source_reaction_product!(source, reaction)
    rps = reaction.reactions_product_samples.find_by(sample_id: source.id)
  ...
  def restore_source_reaction_product!(merge, source)
    return if merge.reaction.reactions_product_samples.exists?(sample_id: source.id)

    attrs = (merge.source_reaction_product_attributes || {}).merge(
      'reaction_id' => merge.reaction_id,
      'sample_id' => source.id,
    )
    ReactionsProductSample.create!(attrs)
  end
  ...
  def validate_reaction_membership!(source, target, reaction)
    products = reaction.reactions_product_samples
    raise MergeError, 'source must be a product of this reaction' unless products.exists?(sample_id: source.id)
