...
class Versioning::Fetchers::ReactionFetcher
  ...
  def call
    ...
    reaction.reactions_samples.with_deleted.with_log_data.each do |reactions_sample|
      ...
      sample_name = (!sample_type.in?(%w[Reactant Solvent]) && sample.short_label).presence || sample.name.presence || sample.external_label.presence || sample.molecule.iupac_name || '-'

      versions += Versioning::Serializers::ReactionsSampleSerializer.call(reactions_sample, ["#{sample_type}: #{sample_name}"])
      versions += Versioning::Serializers::SampleSerializer.call(sample, ["#{sample_type}: #{sample_name}", 'Properties'])
      ...
      versions += sample.elemental_compositions.with_log_data.flat_map do |elemental_composition|
        Versioning::Serializers::ElementalCompositionSerializer.call(elemental_composition, ["#{sample_type}: #{sample_name}", 'Elemental composition'])
      end
      ...
      analyses_container.children.where(container_type: :analysis).with_deleted.with_log_data.each do |analysis|
        versions += Versioning::Serializers::ContainerSerializer.call(analysis, ["#{sample_type}: #{sample_name}", "Analysis: #{analysis.name}"])

        analysis.children.with_deleted.with_log_data.each do |dataset|
          versions += Versioning::Serializers::ContainerSerializer.call(dataset, ["#{sample_type}: #{sample_name}", "Analysis: #{analysis.name}", "Dataset: #{dataset.name}"])

          versions += dataset.attachments.with_log_data.flat_map do |attachment|
            Versioning::Serializers::AttachmentSerializer.call(attachment, ["#{sample_type}: #{sample_name}", "Analysis: #{analysis.name}", "Dataset: #{dataset.name}", "Attachment: #{attachment.filename}"])
          end
        end
      end
    end
