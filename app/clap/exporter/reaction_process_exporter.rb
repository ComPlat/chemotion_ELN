...
module Clap
  module Exporter
    class ReactionProcessExporter < Base
      ...
      def to_clap
        ::Clap::Reaction.new(
          clap_version: CURRENT_CLAP_VERSION,
      ...
      def provenance
        Clap::Exporter::Reactions::ReactionProvenanceExporter.new(model.provenance).to_clap
      end

      def conditions
        Clap::Exporter::Conditions::ReactionConditionsExporter.new(model.initial_conditions).to_clap
      end
      ...
      def reaction_steps
        ...
        process_steps.map.with_index do |rps, idx|
          ReactionProcessStepExporter.new(rps).to_clap(starts_at: start_times[idx])
        end
      end

      def sample_setup
        Clap::Exporter::Reactions::SampleSetupExporter.new(model).to_clap
      end
    end
  end
end
