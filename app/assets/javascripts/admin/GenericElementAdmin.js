...
import { Panel, Table, FormGroup, Popover, FormControl, Button, Row, Col, Badge, Tooltip, OverlayTrigger, InputGroup, Label, ButtonGroup } from 'react-bootstrap';
...
import KlassAttrEditModal from './generic/KlassAttrEditModal';
import KlassAttrNewModal from './generic/KlassAttrNewModal';
...
const editKlassTooltip = <Tooltip id="klass_tooltip">Edit Klass attributes</Tooltip>;
const editKlassTemplateTooltip = <Tooltip id="klass_template_tooltip">Edit Klass template</Tooltip>;
...
export default class GenericElementAdmin extends React.Component {
  ...
  newField(layerKey) {
    ...
    const layer = element && element.properties_template && element.properties_template.layers[layerKey];
    ...
    const newField = { type: 'text', field: newFieldKey, position: 100, label: newFieldKey, default: '' };
  ...
  newOption(key) {
    ...
    const selectObj = element && element.properties_template && element.properties_template.select_options[key];
  ...
  handleUpdateJson(properties_template) {
    const { element } = this.state;
    element.properties_template = properties_template;
    this.setState({
      element,
      showJson: false
    });
  ...
  deleteLayer(key) {
    alert('delete layer:', key);
  }

  onFieldDrop(e) {
    const { element } = this.state;
    const source_key = e.sourceTag.layerKey;
    const target_key = e.targetTag.layerKey;
    const source_layer = element.properties_template.layers[source_key];
    const target_layer = element.properties_template.layers[target_key];

    if (source_layer && target_layer) {

      // const srcIdx = findIndex(source_layer.fields, o => o.field === e.sourceTag.field.field);
      // const tarIdx = findIndex(target_layer.fields, o => o.field === e.targetTag.field.field);
      // console.log('srcIdx:', srcIdx);
      // console.log('tarIdx:', tarIdx);
      // let { fields } = element.properties_template.layers[source_key];
      // fields = filter(element.properties_template.layers[source_key].fields, o => o.field !== e.sourceTag.field.field);
      // fields.splice(tarIdx, 0, e.sourceTag.field);
      // element.properties_template.layers[source_key].fields = fields;
      // console.log('element:', element);
      // this.setState({ element });

      e.sourceTag.field.position = e.targetTag.field.position - 1;

      const { fields } = element.properties_template.layers[source_key];
      const idx = findIndex(fields, o => o.field === e.sourceTag.field.field);
      fields.splice(idx, 1, e.sourceTag.field);
      element.properties_template.layers[source_key].fields = fields;
      this.setState({ element });
      //e.sourceTag.field
      //const sourceIdx = findIndex(groupRec.admins, o => o.id === userRec.id);
    }
  }
  ...
  onFieldInputChange(event, orig, fe, lk, fc, tp) {
    ...
    let fobj = null;
    const layer = (element && element.properties_template && element.properties_template.layers[lk]);
    const { fields } = layer;
    if (layer != null) {
  ...
  onOptionInputChange(event, selectKey, optionKey) {
    const { element } = this.state;
    const options = (element && element.properties_template && element.properties_template.select_options[selectKey]);
  ...
  handlePropShow(element) {
    if (element) {
      const selectOptions = Object.keys(element.properties_template.select_options).map(key =>
        ({ value: key, name: key, label: key })
      );
  ...
  handleSubmit() {
    ...
    //this.setState({ element, showPropModal: false });
  ...
  confirmDelete(delStr, delKey, delRoot) {
    ...
    if (delStr === 'Select') {
      ...
      const sos = element.properties_template.select_options;
      const selectOptions = Object.keys(sos).map(key =>
        ({ value: key, name: key, label: key })
      );
  ...
  renderSelectOptions() {
    ...
    const list = element.properties_template.select_options;

    const optionNodes = Object.keys(element.properties_template.select_options).map(key =>
      (<Panel>{key}</Panel>)
    );
  ...
  renderProperties() {
    ...
    //Object.keys(sortedLayers).filter(p => p !== 'specific_layer')
    ...
    (sortedLayers || []).forEach((layer) => {
      ...
      // const sortedFields = sortBy(((layer && layer.fields) || []), l => l.position);
      // const sortedFields = (layer && layer.fields) || [];
      ...
      const fields = ((layer && layer.fields) || []).map((f, idx) => {
        return (
          <ElementField
            ...
            onMove={(l, f, isUp) => this.onFieldMove(l, f, isUp)}
            onDelete={(delStr, delKey, delRoot) => this.confirmDelete(delStr, delKey, delRoot)}
            onChange={(event, orig, fe, lk, fc, tp) => this.onFieldInputChange(event, orig, fe, lk, fc, tp)}
          />
        );
      }) || [];
  ...
  renderPropPanel() {
    ...
    if (this.state.showPropModal) {
      // let molfile = this.element;
      // molfile = molfile.replace(/\r?\n/g, '<br />');
      return (
        <Panel show={this.state.showPropModal}>
          <Panel.Heading>
            ...
            <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>Save template</Tooltip>}>
              <Button className="button-right" bsSize="xs" bsStyle="warning" onClick={() => this.handleSubmit()}>
    ...
    const tbody = elements && elements.map((e, idx) => (
      <tbody key={`tbody_${e.id}`}>
        <tr key={`row_${e.id}`} id={`row_${e.id}`} style={{ fontWeight: 'bold' }}>
          <td>{idx + 1}</td>
          <td width="12%">
            <OverlayTrigger placement="bottom" overlay={editKlassTooltip} >
              <Button
                bsSize="xsmall"
                bsStyle="info"
                onClick={() => this.editKlass(e)}
              >
          ...
          <td>
            <ButtonGroup>
              <OverlayTrigger placement="bottom" overlay={editKlassTemplateTooltip} >
                <Button bsSize="xs" bsStyle="warning" onClick={() => this.handlePropShow(e)}>
                  <i className="fa fa-file-text" aria-hidden="true" />
                </Button>
              </OverlayTrigger>&nbsp;
              <OverlayTrigger placement="top" overlay={<Tooltip id={uuid.v4()}>Edit template in JSON format</Tooltip>}>
                <Button bsSize="xs" bsStyle="primary" onClick={() => this.showJsonModal(e)}>
