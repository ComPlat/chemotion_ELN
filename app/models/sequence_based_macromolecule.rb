...
class SequenceBasedMacromolecule < ApplicationRecord
  ...
  def self.duplicate_sbmm(base_sbmm)
    if  base_sbmm.uniprot_derivation == 'uniprot'
      find_duplicate_uniprot_sbmm(base_sbmm)
    elsif base_sbmm.uniprot_derivation.in?(%w[uniprot_modified uniprot_unknown])
      find_duplicate_non_uniprot_sbmm(base_sbmm)
    else
      nil
    end
  end
  ...
  def self.find_duplicate_non_uniprot_sbmm(base_sbmm)
    scope = non_uniprot
    scope = scope.where.not(id: base_sbmm.id) if base_sbmm.id
