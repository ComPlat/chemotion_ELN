    ...
    new_analyses = analyses.each do |a|
      if analysis_needs_code?(a)
        if analysis_is_nmr?(a)
          ...
          CodeLog.create(code_type: "bar_code_bruker", value: bar_code_bruker, source: "analysis", source_id: self.id)
        else
          bar_code = Chemotion::CodeCreator.create_bar_code
          qr_code = Chemotion::CodeCreator.create_qr_code
          a["bar_code"] = bar_code
          a["qr_code"] = qr_code
          CodeLog.create(code_type: "bar_code", value: bar_code, source: "analysis", source_id: self.id)
          CodeLog.create(code_type: "qr_code", value: qr_code, source: "analysis", source_id: self.id)
        end
      end
    end
