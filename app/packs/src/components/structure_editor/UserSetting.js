...
const UserSetting = () => {
  ...
  const initProfile = () => {
    UsersFetcher.fetchProfile().then((result) => {
      setEditor(prev => ({ ...prev, default: (result?.data?.structure_editor || DEFAULT_EDITOR) }));
    }).catch((error) => {
      console.log(error);
  ...
  const updateProfile = () => {
    ...
    UsersFetcher.updateUserProfile({ structure_editor: editor.selected }).then((result) => {
      setEditor(prev => ({ ...prev, default: result.data.structure_editor, changing: false }));
    }).catch((error) => {
      console.log(error);
    });
  };
