...
export default class Component extends Sample {
  ...
  setPurity(purity, totalVolume, referenceComponent, lockAmountColumnSolids) {
    if (!Number.isNaN(purity) && purity >= 0 && purity <= 1) {
      this.purity = purity;
    }

    this.handlePurityChanges(totalVolume, referenceComponent, lockAmountColumnSolids);
  }

  handlePurityChanges(totalVolume, referenceComponent, lockAmountColumnSolids) {
    if (lockAmountColumnSolids) {
