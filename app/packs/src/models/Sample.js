...
export default class Sample extends Element {
  ...
  calculateRequiredTotalVolume() {
    ...
    if (!referenceComponent) return 0; // Return 0 or some default value if no reference component
    ...
    if (!amount_l || !concn || concn <= 0) {
      // Key fields are missing or invalid, so we can't perform the calculation
      return 0;
    }
    ...
    if (starting_molarity_value && starting_molarity_value > 0) {
      ...
      requiredTotalVolume = (starting_molarity_value * amount_l * purity) / concn;
    } else if (density && density > 0) {
      // Case 2: If density is given
      requiredTotalVolume = (density * amount_l * purity) / (molecule_molecular_weight * concn);
    }
