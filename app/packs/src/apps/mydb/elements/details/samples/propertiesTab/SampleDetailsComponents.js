...
export default class SampleDetailsComponents extends React.Component {
  ...
  updatedSampleForAmountUnitChange(changeEvent) {
    ...
    const isConcentrationLocked = lockedComponents.includes(currentComponent.id);

    // Handle different units of measurement
    this.handleAmountUnitChange(changeEvent, currentComponent, totalVolume, referenceComponent, isConcentrationLocked);
  ...
  updatePurity(changeEvent) {
    ...
    const { lockAmountColumnSolids, lockedComponents } = ComponentStore.getState();
    ...
    sample.updateTotalVolume(currentComponent.amount_mol, currentComponent.concn, isConcentrationLocked);
