...
export const thirdPartyAppBtn = (attachment, options, defaultOption, onChangingSelection) => (
  <div style={{ float: 'right' }}>
    <select 
      id="dropdown"
      className="button"
      value={defaultOption[attachment.id] || ''}
      onChange={event => onChangingSelection(attachment, event.target.value)}
    >
...
export const thirdPartyAppButton = (attachment, options, onSelection) => (
    ...
    <Dropdown.Menu>
      {options.map((option) => (
        <MenuItem
          key={uuid.v4()}
          eventKey={option.id}
          onClick={() => onSelection(attachment, option)}
