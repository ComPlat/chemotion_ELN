  ...
  EXP_MAP_ATTR =
    # commented out lines are blacklisted attributes
    {
      sample: {
        ...
        color: ["COALESCE(s.sample_details->>'color', s.xref->>'color')", '"color"', 0],
        form: ['s."form"', '"form"', 0],
        height: ["COALESCE(s.sample_details->>'height', s.xref->>'height')", '"height"', 0],
        width: ["COALESCE(s.sample_details->>'width', s.xref->>'width')", '"width"', 0],
        length: ["COALESCE(s.sample_details->>'length', s.xref->>'length')", '"length"', 0],
        storage_condition: ["COALESCE(s.sample_details->>'storage_condition', s.xref->>'storage_condition')", '"storage condition"', 0],
        state: ["COALESCE(s.sample_details->>'state', s.xref->>'state')", '"state"', 0],
