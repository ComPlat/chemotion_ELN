  ...
  class VersionAPI < Grape::API
    ...
    helpers do
      def revert_record(change)
        return Literal.with_deleted.find(change['db_id']) if change['klass_name'] == 'Literal'

        change['klass_name'].constantize.find(change['db_id'])
      end
      ...
      def literals_revertible?(changes)
        changes.select { |change| change['klass_name'] == 'Literal' }.all? do |change|
          ElementPolicy.new(current_user, Literal.with_deleted.find(change['db_id']).element).update?
        end
      end
    end
      ...
      after_validation do
        ...
        if resource == 'revert'
          result = params[:changes].reduce do |res, change|
            res && ElementPolicy.new(current_user, revert_record(change))
          end
