    ...
    resource :attachments do
      ...
      post 'regenerate_spectrum' do
        ...
        Attachment.where(id: pm[:original]).each do |att|
          next unless writable?(att)
          # The frontend buckets ids by filename alone, so a derived curve can arrive here as
          # "original". It is regenerated by its root's own reprocess; resubmitting it would race
          # that. A row that cannot transition (e.g. an LCMS jcamp already :peaked on upload) is
