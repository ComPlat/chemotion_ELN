...
describe 'Coeffiency Reactions' do
  ...
  describe 'Coeffiency First Reaction' do
    ...
    let(:material_first) { create(:sample, name: 'Material', target_amount_value: 0.004, molecule: m1, target_amount_unit: 'g', real_amount_value: nil, real_amount_unit: 'g') }
    let(:material_second) { create(:sample, name: 'Material', target_amount_value: 0.032, molecule: m2, target_amount_unit: 'g', real_amount_value: 0.036, real_amount_unit: 'g') }

    let(:product) { create(:sample, name: 'Product', molecule: m3, real_amount_value: 0.036, real_amount_unit: 'g', target_amount_value: 0) }
    ...
    before do
      ...
      ReactionsStartingMaterialSample.create!(reaction: reaction, sample: material_first, reference: true, equivalent: 1)
      ReactionsStartingMaterialSample.create!(reaction: reaction, sample: material_second, reference: false, equivalent: 0.5035425483256437)
    ...
    it 'Coeffiency 100%', js: true do
      ...
      expect(page).to have_content('Coeff', wait: 3)
      inputs = find_all("input[name='coefficient']")
      inputs[0].set(2)
      inputs[1].set(1)
      inputs[2].set(2)
      find('#submit-reaction-btn').click
    ...
    it 'Coeffiency 50%', js: true do
      ...
      first('i.icon-reaction').click
      first('span.isvg.loaded.reaction').click
      expect(page).to have_content('Coeff', wait: 5)
  ...
  describe 'Coeffiency Second Reaction' do
    ...
    let(:material_first) { create(:sample, name: 'Material', target_amount_value: 30, molecule: m1, target_amount_unit: 'g', real_amount_value: nil, real_amount_unit: 'g') }
    let(:material_second) { create(:sample, name: 'Material', target_amount_value: 20, molecule: m2, target_amount_unit: 'g', real_amount_value: nil, real_amount_unit: 'g') }

    let(:product) { create(:sample, name: 'Product', molecule: m3, real_amount_value: 75, real_amount_unit: 'g', target_amount_value: 0) }
    ...
    before do
      ...
      CollectionsReaction.find_or_create_by!(reaction: reaction, collection: col)
      ReactionsStartingMaterialSample.create!(reaction: reaction, sample: material_first, reference: true, equivalent: 1)
      ReactionsStartingMaterialSample.create!(reaction: reaction, sample: material_second, reference: false, equivalent: 0.2162)
