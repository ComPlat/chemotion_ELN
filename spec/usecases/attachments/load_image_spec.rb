  ...
  describe '.execute!' do
    ...
    context 'with image attachment(tif, already converted)' do
      ...
      it 'size of returned image equals size of converted image' do
        expect(tmp_file.size).to eq File.open(attachment.attachment_data["derivatives"]["conversion"]["id"]).size
      end
    end
    ...
    context 'with image attachment(tif, not yet converted [can happen due migration])' do
      ...
      before do 
        File.delete(attachment.attachment_data["derivatives"]["conversion"]["id"])
      ...
      it 'size of returned image equals size of converted image' do
        expect(tmp_file.size).to eq File.open(updated_attachment.attachment_data["derivatives"]["conversion"]["id"]).size
      end
    end
    ...
    context 'with annotated image' do
      ...
      before do         
        annotation=File.open(Rails.root.join('spec/fixtures/annotations/20221207_valide_annotation_edited.svg')).read
        annotation = annotation.gsub('/46', "/#{attachment.id}")
        annotation_updater.update_annotation(annotation, attachment.id)
      end

      it 'size of returned image equals size of converted image' do       
        expect(tmp_file.size).to eq File.open(updated_attachment.attachment_data["derivatives"]["annotation"]["annotated_file_location"]).size
      end
    end
  end
end
