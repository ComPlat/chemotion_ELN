...
DICT_SMILES = {
  ...
  problematic: [
    ...
    'K',
    '[K]',
    'N#[Cr]123O4N5C(C6=CC=CC=C6O7)=O[Mn]48(O([H])C)(O([H])C)OC9=CC=CC=C9C(N8O%103)=O[Mn]%11%10(Cl)(O([H])C)N(O%122)C(C%13=CC=CC=C%13O%11)=O[Mn]%14%12(O([H])C)(O([H])C)N%15C(C%16=CC=CC=C%16O%14)=O[Mn]57(O([H])C)(Cl)O%151',
  ],
}.freeze
...
DICT_MOLFILE = {
  ...
  # pc_400: 'pc_400.mol',  # mofile from openbabel with pubchem 400 status
}.freeze
...
FactoryBot.define do
  ...
  # @option [String] :from_ctab the ctab (molfile content) to extract custom attributes smiles from (only for type: :smiles)
  ...
  factory :mol_structure, class: String do
    transient do
      cat { :smiles }
      sequence(:index)
      ...
      from { nil } # if set, the smiles will be extracted from the json, ex :pc_400
      ...
      values do
        ...
        file = fixtures_dir.join("#{from}#{from.end_with?('.json') ? '' : '.json'}")
  ...
  factory :smiles, parent: :mol_structure do
    transient do
      cat { :smiles }
      type { nil }
    end
  ...
  factory :molfile, parent: :mol_structure do
    transient do
      cat { :molfile }
      type { :water }
      extension { 'mol' }
  ...
  #                           the .json extension
  # @return [Hash] the json file content
  factory :mol_structure_attributes, class: Hash do
    transient do
      from { nil } # json file to use
  ...
  factory :smiles_set, class: Array do
    ...
    initialize_with do
      values = mol_structure_attributes(from: from, fixtures_dir: fixtures_dir)&.values
      values = values.filter_map { |data| data['type'] == type && (data['smiles'] || data['cano_smiles']) }
  ...
  factory :molfile_set, parent: :smiles_set do
    ...
    initialize_with do
      values = mol_structure_attributes(from: from, fixtures_dir: fixtures_dir)&.values
      values = values.filter_map { |data| data['type'] == type && (data['molfile']) }
      next values if count.zero?
      next values.sample(count) if count < values.size
