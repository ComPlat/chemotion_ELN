      ...
      context "with valid parameters" do
        ...
        describe 'POST /api/v1/samples/subsamples' do
          it 'should be able to split Samples into Subsamples' do
            ...
            s3 = subsamples[0]
            s4 = subsamples[1]
            s3.attributes.except("id", "created_at", "updated_at", "ancestry", "created_by").each do |k, v|
              expect(s1[k]).to eq(v)
            end
            s4.attributes.except("id", "created_at", "updated_at", "ancestry", "created_by").each do |k, v|
              expect(s2[k]).to eq(v)
            end
