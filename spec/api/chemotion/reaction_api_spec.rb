...
describe Chemotion::ReactionAPI do
  ...
  describe 'PUT /api/v1/reactions' do
    ...
    context 'when updating volume and use_reaction_volume_for_concentration' do
      let(:params) do
        {
          'id' => reaction1.id,
          'name' => 'test reaction',
          'volume' => 0.5,
          'use_reaction_volume_for_concentration' => true,
          'container' => reaction_container,
          'materials' => {
            'starting_materials' => [
              {
      ...
      it 'updates the use_reaction_volume_for_concentration attribute' do
        expect(r.use_reaction_volume_for_concentration).to be(true)
      end
    end
  ...
  describe 'POST /api/v1/reactions' do
    ...
    context 'when creating reaction with volume and use_reaction_volume_for_concentration' do
      let(:params) do
        {
          'name' => 'r002',
          'collection_id' => collection1.id,
          'volume' => 0.75,
          'use_reaction_volume_for_concentration' => false,
          'container' => new_root_container,
          'materials' => {
            'products' => [
              'id' => 'd4ca4ec0-6d8e-11e5-b2f1-c9913eb3e335',
      ...
      it 'creates reaction with use_reaction_volume_for_concentration attribute' do
        expect(r.use_reaction_volume_for_concentration).to be(false)
      end
    end
