...
describe Chemotion::WellplateAPI do
  ...
  describe 'GET /api/v1/wellplates/by_generic_element/:id' do
    context 'when element has wellplates' do
      before do
        Labimotion::ElementsWellplate.create!(element: element, wellplate: first_wellplate)
        Labimotion::ElementsWellplate.create!(element: element, wellplate: second_wellplate)
  ...
  describe 'PUT /api/v1/wellplates/by_generic_element/:id' do
    context 'when adding wellplates' do
      ...
      it 'creates associations' do
        expect(response).to have_http_status(:ok)
        expect(Labimotion::ElementsWellplate.where(element_id: element.id).count).to eq(2)
        expect(Labimotion::ElementsWellplate.where(element_id: element.id).pluck(:wellplate_id))
    ...
    context 'when removing wellplates' do
      ...
      it 'removes unselected wellplates' do
        expect(response).to have_http_status(:ok)
        expect(Labimotion::ElementsWellplate.where(element_id: element.id).count).to eq(1)
        expect(Labimotion::ElementsWellplate.find_by(element_id: element.id).wellplate_id).to eq(first_wellplate.id)
      end
    end
    ...
    context 'when removing all wellplates' do
      ...
      it 'removes all associations' do
        expect(response).to have_http_status(:ok)
        expect(Labimotion::ElementsWellplate.where(element_id: element.id).count).to eq(0)
      end
    end
