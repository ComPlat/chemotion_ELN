...
RSpec.describe ElementsPolicy do
  ...
  describe '#allowed?' do
    ...
    context 'if all records in scope are shared with the correct permission level' do
      let(:permission_level) { 4 }

      it 'returns true' do
        policy = described_class.new(user_1, Sample.where(id: [sample_1, sample_2, sample_3, sample_4, sample_5, sample_6]))
