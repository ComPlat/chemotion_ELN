...
describe('CheckBoxList', () => {
  ...
  const wrapper = (items, toggleCheckbox, toggleCheckAll, checkedAll) => mount(
      React.createElement(CheckBoxList, { items, toggleCheckbox, toggleCheckAll, checkedAll })
    );

  it('should match text & checked', () => {
    const expectedText = items.reduce((p, c) => `${p  }  ${  c.text}`, '');
  ...
  //  expect(actual.text()).toInclude(expectedText)
    ...
    for (const i in items) { expectedCheckedness(parseInt(i)); }
  });
