...
describe('SDSAttachmentModal', () => {
  ...
  it('renders modal with correct title', () => {
    ...
    expect(wrapper.find(AppModal).prop('title')).toEqual('Attach Safety Sheet File');
  });
  ...
  it('checks that submit button is present', () => {
    expect(wrapper.find(AppModal).prop('primaryActionLabel')).toEqual('Submit');
  });

  it('checks that close button is present', () => {
    expect(wrapper.find(AppModal).prop('closeLabel')).toEqual('Close');
  });
