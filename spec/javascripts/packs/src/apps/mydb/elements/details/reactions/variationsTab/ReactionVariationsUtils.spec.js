...
describe('ReactionVariationsUtils', () => {
  it('converts units', () => {
    expect(convertUnit(1, 'g', 'mg'))
      .toBe(1000);
    expect(convertUnit(1, 'mg', 'g'))
      .toBe(0.001);
    expect(convertUnit(1, 'l', 'ml'))
      .toBe(1000);
    expect(convertUnit(1, 'ml', 'l'))
      .toBe(0.001);
    expect(convertUnit(1, '°C', '°F'))
      .toBe(33.8);
    expect(convertUnit(1, '°F', '°C'))
      .toBeCloseTo(-17.2, 0.1);
    expect(convertUnit(1, 'Second(s)', 'Minute(s)'))
      .toBeCloseTo(0.0167, 0.00001);
    expect(convertUnit(1, 'Minute(s)', 'Second(s)'))
      .toBe(60);
  });
  it('creates a row in the variations table with selected materials', async () => {
    const reaction = await setUpReaction();

    expect(Object.keys(reaction.variations[0].products).length)
      .toBe(2);
    ...
    expect(Object.keys(row.products).length)
      .toBe(1);

    const nonReferenceStartingMaterial = Object.values(row.startingMaterials)
      .find(
    ...
    expect(row.id)
      .toBe(4);
    expect(row.metadata.analyses)
      .toEqual([]);
    expect(row.metadata.notes)
      .toEqual('');
    ...
    expect(Object.values(row.products)
      .map((product) => product.yield.value))
      .toEqual([100]);
    expect(nonReferenceStartingMaterial.equivalent.value)
      .toBe(1);
    expect(reactant.equivalent.value)
      .toBe(1);
  });
  it('adds missing columns to variations', async () => {
    ...
    let updatedVariations = removeObsoleteColumnsFromVariations(variations, {
      properties: [],
      metadata: [],
    });
    ...
    updatedVariations = addMissingColumnsToVariations({
      materials: {},
      selectedColumns: {
        properties: ['temperature', 'duration'],
    ...
    expect(updatedVariations[0].properties.duration.value)
      .toBe(42);
    expect(updatedVariations[0].properties.temperature.value)
      .toBe(42);
    expect(updatedVariations[0].metadata.notes)
      .toBe('');
  ...
  it('removes obsolete materials from variations', async () => {
    ...
    reaction.variations.forEach((variation) => {
      expect(Object.keys(variation.products))
        .toEqual(productIDs);
    });
  ...
  it('removes obsolete materials from column definitions', async () => {
    ...
    const columnDefinitions = Object.entries(reactionMaterials)
      .map(([materialType, materials]) => ({
    ...
    expect(getColumnDefinitionsMaterialIDs(columnDefinitions, 'startingMaterials'))
      .toEqual(startingMaterialIDs);
  ...
  it('copies a row in the variations table', async () => {
    ...
    expect(copiedRow.id)
      .toBeGreaterThan(row.id);
    expect(copiedRow.metadata.analyses)
      .toEqual([]);
    expect(copiedRow.metadata.notes)
      .toEqual('');
  });
  it('updates a row in the variations table', async () => {
    const reaction = await setUpReaction();
    const row = reaction.variations[0];
    const referenceMaterialID = Object.keys(row.startingMaterials)
      .find(
        (materialID) => row.startingMaterials[materialID].aux.isReference,
      );
    const referenceMaterial = Object.values(row.startingMaterials)
      .find(
        (material) => material.aux.isReference,
      );
    const updatedRow = updateVariationsRow(
      ...
      `startingMaterials.${referenceMaterialID}`,
      {
        ...
        mass: {
          ...
          value: referenceMaterial.mass.value * 10,
        },
      },
      reaction.hasPolymers(),
    );
    expect(Object.values(row.reactants)[0].equivalent.value)
      .toBeGreaterThan(
        Object.values(updatedRow.reactants)[0].equivalent.value,
      );
    expect(Object.values(row.products)[0].yield.value)
      .toBeGreaterThan(
  ...
  it('updates the definition of a column', async () => {
    ...
    expect(getColumnGroupChild(columnDefinitions, 'startingMaterials', field).cellDataType)
      .toBe('material');
    ...
    expect(getColumnGroupChild(updatedColumnDefinitions, 'startingMaterials', field).cellDataType)
      .toBe('equivalent');
  });
  ...
  it('gets column names from variations table', async () => {
    ...
    expect(variationsColumns.metadata)
      .toEqual(expect.arrayContaining(['notes', 'analyses']));

    const emptyVariationsColumns = getVariationsColumns([]);
    expect(emptyVariationsColumns.startingMaterials)
      .toEqual([]);
    expect(emptyVariationsColumns.properties)
      .toEqual([]);
    expect(emptyVariationsColumns.metadata)
      .toEqual([]);
  });
});
